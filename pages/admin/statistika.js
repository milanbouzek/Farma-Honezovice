...
export default function StatistikaPage() {
  ...
  const [charts, setCharts] = useState(() => {
    try {
      ...
      if (raw) {
        ...
        // parsed is array of ids in order — we map to defaultCharts objects
        return parsed
  ...
  const [columns, setColumns] = useState(() => {
    const saved = localStorage.getItem("stats_layout_columns");
    return saved ? Number(saved) : 1;
  });

  // --- Loading data from API (variant 1: API routes) ---
  const fetchData = async () => {
    ...
    try {
      ...
      // compatibility: some endpoints return { data: [...] } or { records: [...] }
  ...
  // pokud kontext ještě neni připravený, nic nezobrazovat
  if (!ready) return null;

  // --- login handler (pomocí context.login) ---
  const handleLogin = async () => {
    const result = login(password);
    if (result?.success) {
      toast.success("✅ Přihlášeno");
      setPassword("");
      // fetchData bude voláno v useEffect podle authenticated
    } else {
      toast.error(result?.message || "Špatné heslo");
    }
  };

  // --- utility pro parsování dat (datumy v DB by měly být ISO nebo date string) ---
  const parseDate = (d) => {
    if (!d) return null;
    // očekáváme formát "YYYY-MM-DD" nebo JS Date string
    const dt = new Date(d);
    if (isNaN(dt)) {
      // pokusíme se převést formát "DD.MM.YYYY"
  ...
  // --- Chart data builders (use current state) ---
  ...
  const getEggsData = () => {
    ...
    (dailyEggs || []).forEach((rec) => {
      ...
      if (key === undefined) return;
      // If your daily table has two columns (standard_eggs, low_cholesterol_eggs), sum them:
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            ...
            // add currency suffix for revenue/profit series
  ...
  // --- header login UI if not authenticated ---
  if (!authenticated) {
    return (
      ...
      <div style={{ display: "grid", gap: 16, gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))` }}>
        {charts.map((chart, index) => (
          <div key={chart.id} className="mb-6 bg-white shadow rounded-xl p-4">
            ...
            {/* pokud data chybí, ukážeme informaci */}
            <div>
