  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStock({ standard_quantity: data.remaining_standard, low_chol_quantity: data.remaining_low_chol });
        setFormData({
          name: "",
          email: "",
          phone: "",
  ...
  return (
    <div>
      <p className="mb-4 text-lg text-gray-700">
        🥚 Aktuálně k dispozici: <strong>{stock.standard_quantity}</strong> standardních vajec (5 Kč/ks), <strong>{stock.low_chol_quantity}</strong> vajec se sníženým cholesterolem (7 Kč/ks)
      </p>
