...
import StockBox from "./components/StockBox"; // propojení se StockBox
...
export default function OrderForm() {
  ...
  // ------------------------------
  // Načtení stavu skladu přes StockBox API
  // ------------------------------
  useEffect(() => {
    let mounted = true;
    const loadStock = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setStock({
          standard_quantity: json.standard_quantity ?? 0,
          low_chol_quantity: json.low_chol_quantity ?? 0,
          standard_price: json.standard_price ?? 0,
          low_chol_price: json.low_chol_price ?? 0,
        });
      } catch (err) {
        if (!mounted) return;
        setStock({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Aktualizace stavu skladu po odeslání
        setStock((prev) => ({
          ...prev,
          standard_quantity: data.remaining?.standard ?? prev.standard_quantity,
          low_chol_quantity: data.remaining?.lowChol ?? prev.low_chol_quantity,
        }));
  ...
  return (
    <div className="max-w-lg mx-auto p-4">
      ...
      <div className="mb-4 text-lg text-gray-700">
        ...
        <p>
          🥚 Standardní vejce: <strong>{stock.standard_quantity}</strong> ks ({stock.standard_price} Kč/ks)
        </p>
        <p>
          🥚 Vejce se sníženým cholesterolem: <strong>{stock.low_chol_quantity}</strong> ks ({stock.low_chol_price} Kč/ks)
        </p>
      </div>
